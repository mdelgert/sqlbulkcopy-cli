...
internal static class CopyCommandFactory
{
    ...
    {
        var configOption = new Option<FileInfo?>("--config", "Optional JSON configuration file path.");
        var sourceConnectionOption = new Option<string?>("--source-connection", "Source SQL Server or Azure SQL connection string.")
        {
            DefaultValueFactory = _ => defaults.SourceConnection
        };
        var sourceQueryOption = new Option<string?>("--source-query", "Source SQL query to execute.")
        {
            DefaultValueFactory = _ => defaults.SourceQuery
        };
        var sourceObjectOption = new Option<string?>("--source-object", "Two-part source object name in schema.object format.")
        {
            DefaultValueFactory = _ => defaults.SourceObject
        };
        var destinationConnectionOption = new Option<string?>("--destination-connection", "Destination SQL Server or Azure SQL connection string.")
        {
            DefaultValueFactory = _ => defaults.DestinationConnection
        };
        var destinationTableOption = new Option<string?>("--destination-table", "Two-part destination table name in schema.table format.")
        {
            DefaultValueFactory = _ => defaults.DestinationTable
        };
        var mappingOption = new Option<string>("--mapping", "Column mapping mode. Supported values: auto.")
        {
            DefaultValueFactory = _ => defaults.Mapping ?? "auto"
        };
        var mapOption = new Option<string[]>("--map", "Explicit source=destination column mapping. Repeat for multiple mappings.")
        {
            AllowMultipleArgumentsPerToken = false,
            DefaultValueFactory = _ => defaults.Map.ToArray()
        };
        var modeOption = new Option<DestinationMode>("--mode", "Destination mode: append, truncate, or delete.")
        {
            DefaultValueFactory = _ => ParseMode(defaults.Mode)
        };
        var confirmDestructiveOption = new Option<bool>("--confirm-destructive", "Required for truncate or delete modes.")
        {
            DefaultValueFactory = _ => defaults.ConfirmDestructive ?? false
        };
        var batchSizeOption = new Option<int>("--batch-size", "Rows per batch for SqlBulkCopy.")
        {
            DefaultValueFactory = _ => defaults.BatchSize ?? 10_000
        };
        var timeoutOption = new Option<int>("--timeout", "Source and metadata command timeout in seconds.")
        {
            DefaultValueFactory = _ => defaults.Timeout ?? 30
        };
        var bulkTimeoutOption = new Option<int>("--bulk-timeout", "SqlBulkCopy timeout in seconds. 0 means no limit.")
        {
            DefaultValueFactory = _ => defaults.BulkTimeout ?? 0
        };
        var notifyAfterOption = new Option<int>("--notify-after", "Progress notification interval in rows.")
        {
            DefaultValueFactory = _ => defaults.NotifyAfter ?? 10_000
        };
        var transactionOption = new Option<bool>("--transaction", "Wrap destination operations in a single transaction.")
        {
            DefaultValueFactory = _ => defaults.Transaction ?? false
        };
        var keepIdentityOption = new Option<bool>("--keep-identity", "Preserve identity values.")
        {
            DefaultValueFactory = _ => defaults.KeepIdentity ?? false
        };
        var keepNullsOption = new Option<bool>("--keep-nulls", "Preserve nulls instead of destination defaults.")
        {
            DefaultValueFactory = _ => defaults.KeepNulls ?? false
        };
        var checkConstraintsOption = new Option<bool>("--check-constraints", "Check destination constraints during bulk copy.")
        {
            DefaultValueFactory = _ => defaults.CheckConstraints ?? false
        };
        var tableLockOption = new Option<bool>("--table-lock", "Acquire a bulk update table lock.")
        {
            DefaultValueFactory = _ => defaults.TableLock ?? false
        };
        var fireTriggersOption = new Option<bool>("--fire-triggers", "Fire insert triggers during bulk copy.")
        {
            DefaultValueFactory = _ => defaults.FireTriggers ?? false
        };
        var useInternalTransactionOption = new Option<bool>("--use-internal-transaction", "Use SqlBulkCopy internal transaction handling.")
        {
